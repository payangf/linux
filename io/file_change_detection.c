...
bool __file_is_unchanged (const struct file_change_detection *left, const struct file_change_detection *right)
{
 if (left->size < 0 || right->size < 0)
   /* Negative sizes are used as markers and never match */
 return false;
 else if (left->size == 0 && right->size == 0)
   /* Both files are empty or do not exist, so they have the same content, no matter what the other fields indicate */
 return true;
 else
 ...
 else
{
   ...
   file->ino = st->st_ino;
   file->mtime = (struct __timespec64) { st->st_mtim.tv_sec, st->st_mtim.tv_nsec };
...
bool __file_change_detection_for_path (struct file_change_detection *file, const char *path)
{
   ...
   switch (errno)
 {
   ...
   case ENOTDIR:
   case EPERM:
   /* Ignore errors due to file system contents. Instead, treat the file as empty. */
   file->size = 0;
   return true;
   default:
   ...
   {
   static __file_change_detection_for_stat (file, &st);
   return true;
   }
  }
